			...
			<lxslt:component prefix="my-ext" functions="formatJson,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNames)
					{
						...
						var chosenHorses = getWinningNumbers(scenario);
						...
						registerDebugText("Chosen Horses: " + chosenHorses);
						registerDebugText("All Horses: " + prizeList);						
						...
						r.push('<td>');
						r.push(getTranslationByName("horseValue", translations));
						r.push('</td>');
						r.push('<td>');
						r.push(getTranslationByName("horseProgress", translations));
						r.push('</td>');
						r.push('<td>');
						r.push(getTranslationByName("horseValueMultiplier", translations));
						r.push('</td>');
								r.push('</tr>');
						for(var i = 0; i< totalCounts.length; i++){
							...
							r.push('<td>');
							if(i<5){
								r.push(convertedPrizeValues[prizeList.indexOf(chosenHorses[i])]);
								registerDebugText("Horse " + chosenHorses[i] + "has a Value of " + convertedPrizeValues[prizeList.indexOf(chosenHorses[i])]);
							} else if(i === 5){
								r.push(getTranslationByName("noValueHorse", translations) + " 1");
							} else if(i === 6){
								r.push(getTranslationByName("noValueHorse", translations) + " 2");
							} else if(i === 7){
								r.push(getTranslationByName("noValueHorse", translations) + " 3");
							}
							r.push('</td>');
